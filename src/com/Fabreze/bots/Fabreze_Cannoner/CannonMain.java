package com.Fabreze.bots.Fabreze_Cannoner;

import com.Fabreze.bots.Fabreze_Cannoner.Branches.IsContinue;
import com.Fabreze.bots.Fabreze_Cannoner.GUI.CannonerController;
...
public class CannonMain extends TreeBot implements EmbeddableUI, ProjectileLaunchListener, InventoryListener, SkillListener {
    //TODO: Add super combat potions
    //TODO: Add chatboxlistener to listen for slayer task ending
    public boolean cannonballcheck = false;
    public boolean foodcheck = false;

    public boolean bankinventory = false;
    public boolean bankcball = false;
    ...
    public int cannonammo = 30;
    public int mincannonammo = Random.nextInt(0, 18);
    ...
    public CannonerController controller = new CannonerController(this);
    ...
    public ObjectProperty<? extends Node> botInterfaceProperty() {
        if (botInterfaceProperty == null) {
            try {
                ...
                final InputStream input = Resources.getAsStream("Fabreze/bots/Fabreze_Cannoner/GUI/CannonerGUI.fxml");
    ...
    public void onProjectileLaunched(ProjectileLaunchEvent var1){
        if (var1.getProjectile().getLaunchPosition().equals(cannon) && var1.getProjectile().getDefinition() != null && var1.getProjectile().getDefinition().getId() == 53){
            cannonammo --;
            Platform.runLater(() -> controller.cannonammo.setText(Integer.toString(cannonammo)));
        }
    }
    ...
    public TreeTask createRootTask() {
        Execution.delayUntil(() -> cannon != null);
